...
from api_notifications import register_notification_api_routes
...
def create_app():
    """Flaskアプリケーションを作成・設定"""
    app = Flask(__name__)
    # テンプレート自動リロードを強制有効化（開発環境）
    app.config['TEMPLATES_AUTO_RELOAD'] = True
    app.config['SEND_FILE_MAX_AGE_DEFAULT'] = 0  # 静的ファイルもキャッシュしない
...
def main():
    ...
    register_notification_api_routes(app)  # 通知API
